...
function handleAnnouncements() {
    var announcementsList = $('.HLAnnouncements ul');
    
    // Ensure the list exists and is not already a slick slider to avoid errors
    if (announcementsList.length && !announcementsList.hasClass('slick-initialized')) {
        // Initialize the Slick Slider
        announcementsList.slick({
            arrows: true,
            dots: false,
            autoplay: false,
        ...
        $('.HLAnnouncements ul li').each(function() {
            ...
            if (!!href) {
                // Check if h3 is not already wrapped in a link to prevent duplicates
                if ($(h3).parent().is('a') === false) {
...
// --- UPDATED FUNCTIONS FOR ASP.NET AJAX ---
...
$(function() {
    ...
    $(document).on('click', '.HLAnnouncements .btn-edit', function() {
        var announcementsList = $('.HLAnnouncements ul');
        
        // If the slider exists and is initialized, unslick it before the postback occurs.
        if (announcementsList.length && announcementsList.hasClass('slick-initialized')) {
            announcementsList.slick('unslick');
